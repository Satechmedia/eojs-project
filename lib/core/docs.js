...
class APIDocumentation {
  ...
  async init() {
    ...
    const swaggerDefinition = {
      // lib/core/docs.js (continued)
      openapi: "3.0.0",
      info: {
        title: this.options.title || packageJson.name || "EOJS API",
        version: this.options.version || packageJson.version || "1.0.0",
        description:
          this.options.description ||
          packageJson.description ||
          "API Documentation for EOJS Application",
      },
      servers: [
        {
          url: this.options.server || "http://localhost:3000",
    ...
    // Find all route files to scan for JSDoc annotations
    const routesDir = path.join(this.appDirectory, "src/routes");
    const routeFiles = [];
    ...
    const controllersDir = path.join(this.appDirectory, "src/controllers");
    ...
    const modelsDir = path.join(this.appDirectory, "src/models");
    const modelFiles = [];
